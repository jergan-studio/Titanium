export class Titanium {
  constructor(options = {}) {
    ...
    this.intensity = allowed.includes(options.intensity)
      ? options.intensity
      : 'medium';

    this.fpsTarget = Number.isFinite(options.fpsTarget)
      ? Math.max(30, Math.min(240, options.fpsTarget))
      : 60;
  ...
  _frame(now) {
    ...
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('titanium:frame', {
        detail: {
          delta,
